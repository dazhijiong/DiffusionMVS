...
class StageNet(nn.Module):
    ...
    def forward(self, features, proj_matrices, depth_values, tmp=2.0,depth_gt_ms=None):
        ...
        if self.training:
            ...
            depth_gt=depth_gt_ms['stage{}'.format(self.stage_idx + 1)]
            print("depth_gt",depth_gt.shape)
            print("depth_values",depth_values.shape)
            
            gt_delta_inv_depth = depth_gt - depth_values#compute the ground truth depth residual x0
            ...
            t = torch.randint(0, self.timesteps, (batch_size,), device=depth_values.device).long()
            ...
            inv_depth_new = depth_values + delta_inv_depth
            ...
            for i in range(self.iters):
                ...
                if self.args['depth_type'] == 'ce' or self.args['depth_type'] == 'was':
                    ...
                    if self.training:
                        ...
                        # regression (t)
                        depth = depth_regression(F.softmax(prob_volume_pre * tmp, dim=1), depth_values=inv_depth_new)
                    # conf
                    ...
                    if self.ndepth >= 32:
                        ...
                        photometric_confidence = prob_volume.max(1)[0]  # [B,H,W]
                delta_inv_depth = delta_inv_depth + depth
