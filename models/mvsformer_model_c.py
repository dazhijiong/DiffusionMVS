...
class StageNet(nn.Module):
    ...
    def forward(self, features, proj_matrices, depth_values, tmp=2.0,depth_gt_ms=None):
        # print("depth_values in stageNet begin",depth_values)
        ref_feat = features[:, 0]
        ...
        if self.args['depth_type'] == 'ce' or self.args['depth_type'] == 'was':
            ...
            if self.ndepth >= 32:
                ...
                photometric_confidence = prob_volume.max(1)[0]  # [B,H,W]
        print("depth_values in stageNet end",depth_values)
...
class TwinMVSNet(nn.Module):
    ...
    def forward(self, imgs, proj_matrices, depth_values, tmp=2.0,depth_gt_ms=None):
        B, V, H, W = imgs.shape[0], imgs.shape[1], imgs.shape[3], imgs.shape[4]
        ...
        for stage_idx in range(len(self.ndepths)):
            ...
            if self.training:
                ...
                inv_depth_new = torch.clamp(inv_depth_new, min=1, max=1000)
                for i in range(self.iters):
                    delta_inv_depth = delta_inv_depth.detach()
                    # print(i, delta_inv_depth.size())
                    inv_depth_new = inv_depth_new.detach()
                    outputs_stage = self.fusions[stage_idx].forward(features_stage, proj_matrices_stage, delta_inv_depth, tmp=tmp,depth_gt_ms=depth_gt_ms)
                    # print("outputs_stage['depth'] before add",outputs_stage['depth'].shape)
                    _, idx = torch.max(outputs_stage["prob_volume"], dim=1)
                    outputs_stage['depth']=outputs_stage['depth']+torch.gather(depth_est, dim=1, index=idx.unsqueeze(1)).squeeze(1).detach()
                    # print("outputs_stage['depth'] after add",outputs_stage['depth'].shape)
                    outputs_stage['depth'] = torch.clamp(outputs_stage['depth'], min=1, max=1000)
                    print("depth_values mid",outputs_stage['depth_values'])
                ...
                for time, time_next in tqdm(time_pairs, desc = 'sampling loop time step'):
                    ...
                    for i in range(self.iters):
                        ...
                        delta_inv_depth = delta_inv_depth.detach()
                        inv_depth_new = inv_depth_new.detach()
                        outputs_stage = self.fusions[stage_idx].forward(features_stage, proj_matrices_stage, inv_depth_new, tmp=tmp,depth_gt_ms=depth_gt_ms)
                        
                        outputs_stage['depth']=outputs_stage['depth']+depth_est
                        outputs_stage['depth'] = torch.clamp(outputs_stage['depth'], min=1, max=1000)
            ...
            # outputs_stage = self.fusions[stage_idx].forward(features_stage, proj_matrices_stage, depth_samples, tmp=tmp,depth_gt_ms=depth_gt_ms)
