    ...
    def _train_epoch(self, epoch):
        ...
        for dl in self.data_loader:
            for batch_idx, sample in enumerate(dl):
                ...
                # print("self.scale_batch_map",self.scale_batch_map)
                ...
                for bi in range(iters_to_accumulate):
                    ...
                    if self.fp16:
                        with torch.cuda.amp.autocast():
                            outputs = self.model.forward(imgs_tmp, cam_params_tmp, depth_values[b_start:b_end],depth_gt_ms=depth_gt_ms)
                    else:
                        outputs = self.model.forward(imgs_tmp, cam_params_tmp, depth_values[b_start:b_end],depth_gt_ms=depth_gt_ms)
                    ...
                    for key in loss_dict:
                        assert torch.isnan(loss_dict[key]).sum() == 0, print(loss_dict[key])
